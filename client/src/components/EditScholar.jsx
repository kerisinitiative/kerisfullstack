...
export default function Record() {
  ...
  async function onSubmit(e) {
    ...
    // Handle array fields
    const majors = form.major.split(",").map(item => item.trim()).filter(item => item);
    const institutions = form.institution.split(",").map(item => item.trim()).filter(item => item);
    majors.forEach(major => formData.append("major[]", major));
    institutions.forEach(institution => formData.append("institution[]", institution));
    ...
    try {
      const response = await fetch(
        params.id ? `http://localhost:5050/record/${params.id}` : "http://localhost:5050/record",
        {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 max-w-6xl">
      <h3 className="text-2xl font-bold mb-6">
        {params.id ? "Edit Scholar Record" : "Create New Scholar Record"}
      </h3>
      ...
      <form onSubmit={onSubmit} className="bg-white shadow-md rounded-lg p-6">
            ...
            <div>
              ...
              {preview && (
                <div className="mt-3">
                  ...
                  {params.id && !(form.image instanceof File) && (
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t">
          <button
            type="button"
            onClick={() => navigate(params.id ? "/admin/scholar-list" : "/scholar")}
          ...
          >
            {params.id ? "Update Record" : "Create Record"}
          </button>
        </div>
      </form>
    </div>
